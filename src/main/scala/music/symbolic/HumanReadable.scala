...
object Interval extends HumanReadable[MVec] {
  ...
  override def apply(v: MVec): String = {
    val intervalInfo = intervals.find(p => p._2 == v.n)
    val intervalName = intervalInfo.map(_._1).getOrElse(v.n)
    val usePerfect = intervalInfo.exists(_._3)
    var adjustmentName = adjustments.find(p => p._2 == v.a).map(_._1).getOrElse(v.a)
